...
> **Stripe for AI Agents.** Drop in an API key. Your agent transacts on-chain through a non-custodial smart account with three-tier policy enforcement (AUTO / GUARD / HUMAN) and pluggable AI Detect for prompt-injection and intent-mismatch.

Running today: end-to-end onboarding, off-chain policy engine, owner-signed approval queue, AI Detect with intent-diff + injection-signature providers, and x402 micropayment fast path. **Base Sepolia · Bun · Next.js 16.**

**Live demo:** https://agentguard.polyoctant.com (Cloudflare tunnel → local dev, may be offline if our laptop is asleep)
...
A **non-custodial control plane** that sits between your AI agent and the chain. The user's owner key never leaves Privy's TEE. The backend only holds **bounded session keys** with on-chain enforced limits. Three layers of defense in front of every transfer:

1. **Hard on-chain limits** — ZeroDev Kernel session-key validators reject anything over policy.
2. **Off-chain policy engine** — whitelist, daily caps, recipient familiarity, anomaly bumps.
3. **AI Detect** — pluggable providers (built-in `intent-diff` + `injection-signature`, GPT-4o-mini) flag prompt injection and intent mismatches before the chain even sees the UserOp.

Every transfer routes to one of three tiers based on policy + detection: **AUTO** (agent signs in <1 s), **GUARD** (backend signs after policy clearance), **HUMAN** (owner approves via a Privy popup in the dashboard).
...
```
   ...
   │                                                              │
   │   auth  →  Policy Engine  →  AI Detect  →  Router            │
...
You need: [Bun](https://bun.sh), a [ZeroDev](https://dashboard.zerodev.app) project on Base Sepolia with **Sponsor All** gas policy, a [Privy](https://dashboard.privy.io) app with Base Sepolia + embedded wallets, and an OpenAI key (optional — without it, AI Detect returns `safe`).
...
// AI Detect compares the user's stated intent against the proposed UserOp.
...
```
 ...
 AI Detect        OpenAI gpt-4o-mini · structured JSON output
...
agentguard/
├── apps/
│   ├── backend/       Elysia API · policy engine · AI Detect · execute pipeline
